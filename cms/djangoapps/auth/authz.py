...
def get_course_groupname_for_role(location, role):
    ...
    if isinstance(location, Location):
        groupname = '{0}_{1}'.format(role, location.course)
        if len(Group.objects.filter(name=groupname)) > 0:
            return groupname

    groupname = '{0}_{1}'.format(role, location.course_id)
    if len(Group.objects.filter(name=groupname)) > 0:
        return groupname
    elif isinstance(location, CourseLocator):
        groupname = '{0}_{1}'.format(role, location.as_old_location_course_id)
